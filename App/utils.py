...
def create_input_data(Age, Menopausal, Hormone_Resistance, ER, PgR, HER2, Ki67, Histotype, M_Brain, M_Liver, M_Bone, M_Peritoneal, Bone_Only, Sinc):
    input_data = pd.DataFrame({
        ...
        'Hormone_Resistance_Primary': [1 if Hormone_Resistance == "Primary Resistant" else 0],
        'Hormone_Resistance_Secondary': [1 if Hormone_Resistance == "Secondary Resistant" else 0],
        'ER': [ER], 
        'PgR': [PgR],
        'HER2': [1 if HER2 == "Low" else 0],
        'Ki67': [Ki67],
...
def shap_explanation(sample_data, input_data_ord, model):
    background = joblib.load("App/shap_background_gbm.pkl")
